...
bool OT_API::Init()
{
	...
	if (bGetDataFolderSuccess) OTRunnableAsProgram::OpenPid(strPIDPath); // creating PID now - after configuration was loaded
	// TODO make this operationg atomic with checking if the pid was not yet existing (switch to anchors)

	if (!OTRunnableAsProgram::IsPidOpen()) { this->m_bInitialized = false; return false; }  // failed loading
